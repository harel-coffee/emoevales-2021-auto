...
class DeepLearningTechniques (BaseModel):
    ...
    all_available_features = ['lf', 'se', 'be', 'we', 'ne', 'cf', 'bf', 'pr']
    ...
    def train (self, force = False, using_official_test = True):
        ...
        available_labels = self.dataset.get_available_labels () if task_type == 'classification' else []
        ...
        if self.dataset.dataset == 'emoeval':
            params_batch_size = [32, 64, 128, 256, 512]

        if self.dataset.dataset == 'haha':
            params_batch_size = [256, 512]
                
        if self.dataset.dataset == 'satire':
            params_batch_size = [256, 512]
        ...
        if 'classification' == task_type:
            metrics_to_show = ['best', 'epochs', 'accuracy', 'val_accuracy']
        ...
        # @link https://www.tensorflow.org/tutorials/structured_data/imbalanced_data
        class_weights = dict (enumerate (class_weight.compute_class_weight ('balanced', available_labels, self.dataset.get_split (df, 'train')['label']))) \
        ...
        metric_of_the_best_model = float ('-inf') if task_type == 'classification' else float ('inf')
        ...
        train_resume = {
            ...
            'class_weights': str (json.dumps (class_weights)),
        ...
        for counter, params in enumerate (param_list):
            ...
            early_stopping = self.get_early_stopping_callback (
                patience = self.get_patience_per_achitecture (params['architecture'])
            )
            ...
            str_metrics = ' '.join ([f'{float ("-inf"): >{max (6, len (metric))}}' for metric in metrics_to_show])
            
            
            # @var pbar_description String
            pbar_description = "{:>3}".format (str (counter + 1)) + ": " + str_metrics + " " + str_features
            
            
            # @var pbar a TQDM progress bar to log the process
            pbar = tqdm (desc = pbar_description)
            ...
            model_results = {
                'best': 0,
                'epoch': early_stopping.stopped_epoch or params['epochs']
            }
            ...
            if ('classification' == task_type and model_results[main_metric] > metric_of_the_best_model) or ('regression' == task_type and model_results[main_metric] < metric_of_the_best_model):
            ...
            # Start
            pbar.set_description ("{:>3}".format (str (counter + 1)) + ": " + str_metrics + " " + str_features)
    ...
    def predict (self, using_official_test = False, callback = None):
        ...
        # model_filename = self.dataset.get_working_dir (self.dataset.task, 'models', 'deep-learning', feature_key, 'model_with_val.h5')
        ...
        try:
            best_model = tensorflow.keras.models.load_model (model_filename, compile = False)
        except:
    ...
    def get_y_real (self, dataset):
        ...
        y_real = [labels[int (item)] for item in y_real]
    ...
    def get_early_stopping_metric (self):
        ...
        # @var early_stopping_monitor String
        early_stopping_monitor = 'val_loss' if self.dataset.get_task_type () == 'classification' else 'val_rmse'
        
        if self.dataset.is_imabalanced ():
            early_stopping_monitor = 'val_prc'
            
        return early_stopping_monitor


    def get_patience_per_achitecture (self, architecture):
